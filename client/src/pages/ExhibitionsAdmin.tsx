...
export default function ExhibitionsAdmin() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateExhibition.mutate({ id: editingId, data: formData });
    } else {
      createExhibition.mutate({ ...formData, displayOrder: exhibitions.length });
    }
  };
      ...
      <main className="flex-1 flex flex-col items-center mt-24 max-w-2xl mx-auto w-full">
        <div className="border border-black p-6 w-full bg-white mb-8">
          <div className="font-mono text-xs uppercase mb-4 bg-black text-white inline-block px-2 py-1">
            {editingId ? "edit archive entry" : "add new archive entry"}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="font-mono text-xs block mb-1 lowercase">location</label>
              <input
                type="text"
                value={formData.location}
                onChange={(e) => setFormData(prev => ({ ...prev, location: e.target.value }))}
                placeholder="e.g. gallery name, city"
                className="w-full border border-black p-2 font-mono text-sm focus:outline-none"
        ...
        <div className="border border-black p-6 w-full bg-white mb-8">
          <div className="font-mono text-xs uppercase mb-4 bg-black text-white inline-block px-2 py-1">
            existing archive entries ({exhibitions.length})
          </div>
                      ...
                      <div className="flex-1 font-mono text-xs lowercase">
                        ...
                        <div className="flex gap-2 mt-2 flex-wrap">
                          ...
                          <button
                            ...
                            data-testid={`button-edit-${exhibition.id}`}
                          >
                            edit
                          </button>
                          <button
                            ...
                            data-testid={`button-delete-${exhibition.id}`}
                          >
                            delete
                          </button>
                        </div>
